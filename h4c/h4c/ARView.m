...
- (void)updatePlacesOfInterestCoordinates
{
	...
	for (NSData *d in [orderedDistances reverseObjectEnumerator]) {
        ...
        show.frame=poi.view.frame;
        [show addTarget:self action:@selector(show:) forControlEvents:UIControlEventTouchUpInside];
        show.autoresizingMask=UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
        [poi.view addSubview:show];
        orig=poi.view.frame;
        poi.view.y+=arc4random()%150-75;
...
-(void)show:(UIButton *)sender{
    ...
    [UIView animateWithDuration:.2 animations:^{
        clicked.superview.layer.zPosition=1;
        clicked.superview.frame=orig;
    }];
    ...
    [(MPPointDescView *)clicked.superview showInfo];
    clicked.superview.layer.zPosition=1111111;

    [UIView animateWithDuration:.2 animations:^{
        clicked.superview.height=160;
;
    }];
